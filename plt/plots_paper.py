...
Both modes share the same machinery: load Bayes factors from the comparison
CSV, load AGNI outputs, compute the planet/star contrast spectrum, and
overlay them in a single figure grouped by Bayesian consistency with the
observed JWST spectrum.

Usage:
    python plt/plot_thesis.py --planet gj486b --mode atmo
    python plt/plot_thesis.py --planet gj486b --mode surface
    python plt/plot_thesis.py --planet gj367b --mode atmo --pressure 100
"""
...
def load_bayes_delta_lnZ(planet: str, mode: str, fixed_key: str, keys: Iterable[str]) -> dict:
    ...
    filtered = df[mask]
    return dict(zip(filtered[varying_col], filtered["\u0394lnZ"]))
...
def load_model_contrasts(planet: str, mode: str, fixed_key: str, varying_keys: Iterable[str],
    ...
    for key in varying_keys:
        # In atmo mode: surface=fixed_key, atmosphere=key
        # In surface mode: surface=key, atmosphere=fixed_key
        if mode == "atmo":
...
def plot_contrasts(
    ...
    # Bayes factors and model contrasts
    ...
    delta_lnZ = {k: delta_lnZ[k] for k in contrasts}  # drop those with no AGNI output
    ...
    # Per-category indexing (avoids the colour-map overflow bug)
    for i, key in enumerate(accepted):
        _plot_one(key, blue_cmap(i + 1))
    ...
    # Legends: models on the left, references on the right
...
if __name__ == "__main__":
    ...
    # Resolve keys, labels, highlights per mode
    if args.mode == "atmo":
